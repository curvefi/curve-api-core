...
const getConfigs = memoize(async (returnOnlyEnabledNetworkIds = true) => {
  ...
  const filePaths = ['deployments/prod/fantom.yaml']; // One empty item for one loop
  ...
  const configs = await sequentialPromiseMap(filePaths, async (filePath) => {
    ...
    const yamlFile = fs.readFileSync('/Users/p/workspace/curve-api-lite/constants/configs/fantom.yaml', 'utf8');
